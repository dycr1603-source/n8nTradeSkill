...
// ────────────────────────────────────────────────────────────────────────────────
// FIX #1: calcTimeLockSL mejorado — retiene 0.8R mínimo
// ────────────────────────────────────────────────────────────────────────────────
function calcTimeLockSL(positionSide,entryPrice,currentPnL,hoursOpen,currentPct,tick,atr=null){
  let bestRule=null;
  for(const rule of TIME_RULES){
    if(hoursOpen>=rule.h&&currentPct>=rule.pct) bestRule=rule;
  }
  if(!bestRule)return{sl:null,rule:null};
  const MIN_R_TO_KEEP=0.8;
  const initialRisk=positionSide==='SHORT'?entryPrice*0.02:entryPrice*0.02;
  let protectionAmount=initialRisk*MIN_R_TO_KEEP;
  if(atr&&atr>0){
    const atrProtection=atr*0.3;
    protectionAmount=Math.max(protectionAmount,atrProtection);
  }
  const ruleProtection=currentPnL*bestRule.keep;
  protectionAmount=Math.max(protectionAmount,ruleProtection*0.8);
  const sl=positionSide==='SHORT'?roundTick(entryPrice-protectionAmount,tick):roundTick(entryPrice+protectionAmount,tick);
...
const results=await Promise.all(Object.keys(positions).map(async(symbol)=>{
  ...
  try{
    ...
    if(currentPct>0&&stageWeight('TIME_LOCK')>=currentWeight&&!candidateSL){
      // FIX #1: Pasar 'atr' como parámetro
      const{sl:tlSL,rule}=calcTimeLockSL(positionSide,entryPrice,currentPnL,hoursOpen,currentPct,tick,atr);
      if(tlSL!==null){
