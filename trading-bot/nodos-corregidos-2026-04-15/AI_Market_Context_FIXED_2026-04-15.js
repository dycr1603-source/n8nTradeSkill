...
// ── FIX RSI: SHORT<25, LONG>78 (era SHORT<30, LONG>70) ───────────────────────
const rsiDangerous  = (direction === 'SHORT' && rsi < 25) || (direction === 'LONG' && rsi > 78);
...
const basePassAI = biasAligns && !rangingBlock && scoreAdjusted >= dynamicThreshold && !rsiDangerous && !volumeSpike;
const historyNearThresholdPass =
  historyOverrideRelief > 0 &&
  biasAligns &&
  !rangingBlock &&
  !rsiDangerous &&
...
if(passAI){
  return [{
    json: {
      ...
      filters: { visionReject:false, visionLate:false, rsiDangerous, volumeSpike, biasAligns, rangingBlock, historyNearThresholdPass }
    }
  }];
}
...
let skipReason = '';
if(rsiDangerous){
  const rsiLimit = direction === 'SHORT' ? 25 : 78;
...
for(const fb of fallbacks){
  ...
  // FIX RSI fallbacks: SHORT<25, LONG>78
  const fbRsiDangerous = (fbDir === 'SHORT' && fbRsi < 25) || (fbDir === 'LONG' && fbRsi > 78);
...
return [{
  json: {
    ...
    filters: { visionReject:false, visionLate:false, rsiDangerous, volumeSpike, biasAligns, rangingBlock, historyNearThresholdPass:false }
  }
}];
