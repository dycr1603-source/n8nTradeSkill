...
const KILL_SWITCH_FILE = process.env.N8N_TRADING_KILL_SWITCH_FILE || '/tmp/aterum_trading_disabled.flag';
const tradingDisabled = process.env.N8N_TRADING_DISABLED === '1' || fs.existsSync(KILL_SWITCH_FILE);
...
if(tradingDisabled){
  return [{
    json: {
      ...
      passAI: false,
      finalScore: 0,
      skipReason: `Trading disabled by kill switch (${KILL_SWITCH_FILE})`,
...
// Recovery mode: imagen ya aporta una capa extra, pero no reducimos threshold por defecto.
dynamicThreshold = Math.max(60, dynamicThreshold);
  ...
  if(historyPolicyMatch){
    ...
    if(tfOk && openOk && volOk && rsiOk){
      if(RECOVERY_MODE){
        ...
        console.log(`[${symbol}][IMG] Recovery mode: policy observada pero sin bajar threshold para ${direction}|${macroRelation}|4h=${tf4hStatus}`);
      } else {
...
let aiResult = {
  approve: false, regime:'NEUTRAL', direction_bias:'NEUTRAL',
  recommended_leverage:1, confidence_adjustment:-20,
  sl_multiplier:1.5, tp_multiplier:2.0, risk_reduction:0,
  key_risk:'AI unavailable', reasoning:'fallback defaults — bloqueado en recovery mode'
};
...
if(!passAI){
  if(usedAIFallback)
    skipReason = `AI fallback bloqueado en recovery mode — score=${scoreWithLatePenalty} threshold=${dynamicThreshold}`;
...
return [{
  json: {
    ...
    decisionAudit: {
      stage: 'AI Market Context Image',
      decision: passAI ? 'approved' : 'blocked',
      reason: passAI ? 'passAI' : skipReason,
      macroBias,
      tf4hStatus,
      dynamicThreshold,
      finalScore: scoreWithLatePenalty,
      filters: { visionReject, visionLate, rsiDangerous, volumeSpike, biasAligns, rangingBlock, aiFallback: usedAIFallback }
    },
    filters: { visionReject, visionLate, rsiDangerous, volumeSpike, biasAligns, rangingBlock, aiFallback: usedAIFallback }
  }
}];
